...
function buildDeviceComponents({ projectConfig, buildId, onDiagnostic = diagnostics_1.logDiagnosticToConsole, }) {
    ...
    const resources = vinyl_fs_1.default.src('./resources/**', { base: '.' });
    const pofiles = vinyl_fs_1.default.src('./i18n/**/*.po', { base: '.' });
    return projectConfig.buildTargets.map((platform) => {
        const sourceMap = collectComponentSourceMaps_1.default();
        return new pumpify_1.default.obj(merge_stream_1.default(new pumpify_1.default.obj(baseJS, sourceMap.collector(componentTargets_1.ComponentType.DEVICE, platform)), new pumpify_1.default.obj(resources, buildDeviceResources(projectConfig, buildTargets_1.default[platform], onDiagnostic)), new pumpify_1.default.obj(pofiles, compileTranslations_1.default())), componentManifest_1.makeDeviceManifest({ projectConfig, buildId }), zip_1.default(`device-${platform}.zip`, { compress: false }), sourceMap.emitter);
    });
}
...
function buildProject({ onDiagnostic = diagnostics_1.logDiagnosticToConsole } = {}) {
...
function build({ dest = vinyl_fs_1.default.dest('./build'), onDiagnostic = diagnostics_1.logDiagnosticToConsole, } = {}) {
    return new Promise((resolve, reject) => {
        new pumpify_1.default.obj(buildProject({ onDiagnostic }), dest)
            .on('error', reject)
            .on('finish', resolve);
    }).catch((e) => {
        if (buildError_1.isPluginError(e) && buildError_1.isProjectBuildError(e)) {
            onDiagnostic(buildError_1.convertPluginErrorToDiagnostic(e));
